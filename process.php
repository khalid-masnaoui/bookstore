...
if(isset($_POST["Create"])){
    ...
    if(array_filter($errors)){
        ...
        try{
            ...
            $stmt=$db->prepare("INSERT INTO BOOKS (user_id,book_name,publisher,book_price) VALUES (:user,:name,:publisher,:price)");
            $stmt->execute([":user"=>$user_id,":name"=>$name,":publisher"=>$publisher,":price"=>$price]);
            $_SESSION["action"]="ADD";
            header("location:bookstore.php");
...
if(isset($_POST["Update"])){
    ...
    //checking for errors (just empty name/publisher field)
    if(empty($name)){
        $errors["name"]=" * field is required";
    }
    if(empty($publisher)){
